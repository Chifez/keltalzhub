        ...
        >
          {links.map((items, idx) => (
            <li key={idx} className="group relative pb-1">
              ...
              <span
                className={`absolute bottom-0 transition-all duration-200 left-0 h-1 w-0 rounded-md lg:group-hover:w-full bg-[#000B6A] ${
                  currentRoute === items.link ? 'w-full bg-[#000B6A]' : ''
                }`}
              ></span>
            </li>
          ))}
